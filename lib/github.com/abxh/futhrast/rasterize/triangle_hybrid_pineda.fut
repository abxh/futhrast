...
module CustomHybridPinedaTriangleRasterizer (O: HybridPinedaTriangleRasterizerOptions) : TriangleRasterizerSpec = \(V: VaryingSpec) ->
  {
    ...
    def rasterize 'target [h] [n] [w]
      ...
      let (small_partition, other_partition) = bin_rasterize {w,h} tris
      let (tile_ids, tri_idxs) =
        other_partition
        |> coarse_rasterize {w,h} tris
        |> radix_sort_by_key (.0) (i32.i64 num_bits_to_sort) u32.get_bit
        |> unzip
      let dvis_buffer =
        rasterize_tiled dvis_buffer tri_infos (tile_ids, tri_idxs)
      let dvis_buffer =
        rasterize_small_triangles dvis_buffer tri_infos small_partition
      let (is, xs) =
        dvis_buffer
        |> flatten
        |> zip (iota (h * w))
...
module HybridPinedaTriangleRasterizerTest = {
  ...
  def test_triangle_hybrid_pineda [n] (h: i64) (w: i64) (vs: [n]((f32, f32), (f32, f32), (f32, f32))) : [h][w]i32 =
