...
module TiledTriangleRasterizer : TriangleRasterizerSpec = \(V: VaryingSpec) ->
  {
    ...
    def rasterize 'target [h] [w]
      ...
      let bins_w = (w + bin_size - 1) >> bin_shift
      let ne_bin = tabulate (bin_size * bin_size) (const ne_depth)
      let merge_bins b0 b1 = map2 depth_select b0 b1
      ...
      let depth_buffer =
        ...
        |> expand (\i ->
                     ...
                     let num_chunks = 1 `i64.max` (frag_count + frag_block_size - 1) >> frag_block_shift
                     in num_chunks)
                  (\i chunk_index ->
                     let is_segment_start = chunk_index == 0
                     let bin_index = bin_idxs[i]
                     let frag_count_rem = bin_counts[i] - chunk_index * frag_block_size
                     let frag_count = frag_block_size `i64.min` frag_count_rem
                     let global_offset = bin_offsets[i] + chunk_index * frag_block_size
                     in (is_segment_start, bin_index, frag_count, global_offset))
        |> map (\(is_segment_start, bin_index, frag_count, global_offset) ->
                  let bin_xmin = (i64.u16 bin_index %% bins_w) << bin_shift
                  let bin_ymin = (i64.u16 bin_index / bins_w) << bin_shift
                  let f frag_index =
                    if frag_index < frag_count
                    then let global_frag_index = global_offset + frag_index
                         let (y, x) = is[global_frag_index]
                         let depth = depth_values[global_frag_index]
                         let (pixel_y, pixel_x) = (y - bin_ymin, x - bin_xmin)
                         in ((pixel_y << bin_shift) + pixel_x, depth)
                    else (-1, ne_depth)
                  let (is, depth_values) =
                    tabulate frag_block_size f
                    |> unzip2
                  let bin_buf = reduce_by_index (copy ne_bin) depth_select ne_depth is depth_values
                  in (is_segment_start, bin_buf))
        |> unzip
        |> uncurry (segmented_reduce merge_bins ne_bin)
        |> sized (length bin_idxs)
        |> flatten
