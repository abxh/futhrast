...
module mk_line_rasterizer : mk_line_rasterizer_spec = \(V: VaryingSpec) ->
  {
    ...
    def get_line_size ((v0, v1): (pfragment V.t, pfragment V.t)) =
      let {x = x0, y = y0} = v0.pos
      let {x = x1, y = y1} = v1.pos
      in 1 + i64.f32 (f32.max (f32.abs (x1 - x0)) (f32.abs (y1 - y0)))

    def get_point_in_line ((v0, v1): (pfragment V.t, pfragment V.t)) (i: i64) : pfragment V.t =
      ...
      in if f32.abs (p0.x - p1.x) > f32.abs (p0.y - p1.y)
         then let dir = compare_f32 (p0.x) (p1.x)
              ...
              let pos = {x = p0.x + f32.i64 (dir * i), y = p0.y + sl * f32.i64 i}
              ...
              let pos = {x = p0.x + sl * f32.i64 i, y = p0.y + f32.i64 (i * dir)}
              let t = f32.from_fraction i (get_line_size (v0, v1) - 1)
              let Z_inv = f32.lerp v0.Z_inv v1.Z_inv t
              let depth = lerp_affine_f32 Z_inv (v0.depth, v0.Z_inv) (v1.depth, v1.Z_inv) t
              let attr = lerp_affine_attr Z_inv (v0.attr, v0.Z_inv) (v1.attr, v1.Z_inv) t
