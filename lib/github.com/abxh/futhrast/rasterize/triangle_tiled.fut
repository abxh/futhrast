...
module TiledTriangleRasterizer : TriangleRasterizerSpec = \(V: VaryingSpec) ->
  {
    ...
    def fine_rasterize [n]
      ...
      let (active_tile_counts, active_tiles) =
        segmented_reduce (\(s0, i0) (s1, i1) -> (s0 + s1, (i0.0 `u16.max` i1.0, i0.1 `u8.max` i1.1)))
    ...
    def calc_segment_flags_u16_u8 [n] (is: [n](u16, u8)) : [n]bool =
      if n == 0
    ...
    def coarse_rasterize [n]
      ...
      let arr =
        ...
        |> zip (iota n)
      let szs = map sz arr
      let bin_counts = segmented_reduce (+) 0 bin_flags szs |> filter (!= 0)
      let (bin_indices, tile_indices, tri_indices) =
        repl_segm_iota szs
        |> uncurry (map2 (\i j -> get arr[i] j))
        |> segmented_radix_sort_by_key (.1) (i32.i64 num_bits) u8.get_bit bin_counts
        |> unzip3
      let tile_flags = calc_segment_flags_u16_u8 (zip bin_indices tile_indices)
      in (tile_flags, bin_indices, tile_indices, tri_indices)
    ...
    def rasterize 'target [h] [w]
      ...
      let f i =
        ...
        let f pixel_index =
          ...
          let y = pixel_y + tile_ymin
          in depth_buffer[y, x]
        in loop depth_tile = tabulate (fine_size * fine_size) f
      ...
      let depth_buffer =
        ...
        |> scatter (copy (flatten depth_buffer))
                   (tabulate (length active_tiles * (fine_size * fine_size)) g)
        |> unflatten
