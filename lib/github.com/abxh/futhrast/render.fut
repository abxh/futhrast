...
module type ProgramSpec = {
  ...
  -- | user-specified target module
  module Target: TargetSpec
  ...
  val on_vert : uniform_t -> vertex_t -> fragment Varying.t

  -- | on quad-fragment function (fragment shader function)
  val on_qfrag : uniform_t -> [4]pfragment Varying.t -> [4]Target.t
}
...
module mk_render (Config: ConfigSpec) (Program: ProgramSpec) = {
  local module Fragment = derive_fragment_ops (Program.Varying)
  local type uniform_t = Program.uniform_t
  local type vertex_t = Program.vertex_t
  local type varying_t = Program.Varying.t
  local type target_t = Program.Target.t
  local type~ framebuffer_t = framebuffer [Config.tile_size] target_t
  ...
  def render ((on_vert, on_qfrag): (on_vert_t, on_qfrag_t))
    ...
    in match d.primitive_type
       case #triangles ->
         ...
         let stw_f = Fragment.map_screen_to_window {w = fb.w, h = fb.h}
