...
-- > :img main ($loaddata "monkey.in")

-- > :img main ($loaddata "penger.in")

-- > :img main ($loaddata "dragon.in")

-- > :img main ($loaddata "lucy.in")

-- > :img main ($loaddata "armadillo.in")
...
import "../../lib/github.com/abxh/futhrast/rasterize/triangle_imm_pineda"
...
module R = CustomRenderSetup ImmPinedaTriangleRasterizer Varying
...
def main [n] (vx: [n]f32, vy: [n]f32, vz: [n]f32, inds: []i64) =
  ...
  let n = length inds / 3
  let prop = 0.25
  let n' = i64.f32 (f32.ceil (f32.i64 n * prop)) * 3
