...
module lys : lys with text_content = lys_text_content.text_content = {
  ...
  local
  def on_vertex (s: state) (v: ((f32, f32, f32), u32)) : vertex_out Varying.t =
  ...
  def render (s: state) : [][]argb.colour =
    ...
    let verts =
      indices inds |> map (\i -> (verts[inds[i]], s.colours[inds[i / 3] %% 1024]))
